...
public class CharactersPresenter implements CharactersContract.Presenter {
  ...
  @Override
  public void loadCharacters(String term) {
    mView.showLoading();
    mUseCaseHandler.execute(mGetCharacters,
                            new GetCharacters.RequestValues(term, 0, PAGE_SIZE),
                            new UseCase.UseCaseCallback<GetCharacters.ResponseValue>() {

      @Override
      public void onSuccess(GetCharacters.ResponseValue response) {
  ...
  @Override
  public void loadMoreCharacters(String term, int offset) {
    mView.showLoading();
    mUseCaseHandler.execute(mGetCharacters,
            new GetCharacters.RequestValues(term, offset, PAGE_SIZE),
            new UseCase.UseCaseCallback<GetCharacters.ResponseValue>() {
